  ...
  <v-layout row justify-center>
    ...
    <v-dialog v-model="dialog" max-width="900px">
      <v-stepper v-model="e1">
        <v-stepper-header>
          <v-stepper-step :complete="e1 > 1" step="1">HYDROCYCLONE OVERFLOW PROPERTIES</v-stepper-step>
          <v-divider></v-divider>
          <v-stepper-step :complete="e1 > 1" step="2">HYDROCYCLONE OVERFLOW EFFICIENCY</v-stepper-step>
          <v-divider></v-divider>
        </v-stepper-header>
        <v-stepper-items>
          <v-stepper-content step="1">
            <v-card>
              <v-flex>
                <span style="color:orange">Solid(t/h) : {{$store.getters.hydrocyclone_of_solid}} </span>
              </v-flex>
              <v-flex>
                <span style="color:orange">Water(t/h) : {{$store.getters.hydrocyclone_of_water}} </span>
              </v-flex>
              <v-flex>
                <span style="color:orange">SR :  </span>
                <input v-model="hydrocyclone_of_SR" @input="changeSR" placeholder="doldur" />
              </v-flex>
              <v-flex>
              ...
              <v-flex>
                <span style="color:orange">f' : {{$store.getters.hydrocyclone_of_f_ustu}} </span>
              </v-flex>   
            </v-card>
          </v-stepper-content>
